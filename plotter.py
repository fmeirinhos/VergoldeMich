...
class Plotter(object):
    ...
    def portfolio_change(self, ax):
        ...
        """
        self.perf.loc[
            :, ['algorithm_period_return']].plot(ax=ax)
        ...
        self.perf.loc[
            :, ['price_change']].plot(ax=ax)
        # self.perf.loc[:, ['price']].plot(
        #     use_index=True, y=(price - price0) / price0, ax=ax)
        # ax.plot(self.perf.loc[:, ['price']].index.to_pydatetime(),
        #         (price - price0) / price0, label='price_change')
